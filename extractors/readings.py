...
def extract_reading_info(ocr_text):
    ...
    # Pre-process: Join numbers split by a single space (e.g. "1842 9" -> "18429")
    # Do NOT join if it starts with 2024, 2025, 2026 (years)
    processed_text = re.sub(r'\b(\d{1,4})\s+(\d{1,3})\b', lambda m: m.group(1)+m.group(2) if (2 <= len(m.group(1)+m.group(2)) <= 6 and m.group(1) not in ['2024', '2025', '2026']) else m.group(0), ocr_text)
